...
This repo is an implementation of the **LLM Wiki** pattern: a persistent, compounding research knowledge base that Codex builds and maintains. The "code" here is almost entirely Codex skill workflow documentation in `.agents/skills/llm-wiki/`. The user curates sources, asks questions, and directs analysis; the LLM owns summarizing, cross-referencing, and bookkeeping.

Two roles to keep distinct:
- **This repo (code)** — Codex skills, workflow references, conventions. Tracked in git.
...
```
.agents/
  active-kb          # Single-line basename pointer to the active KB (gitignored, per-clone; written only by setup/use workflows)
  skills/
    llm-wiki/        # Codex skill for setup/use/ingest/query/wiki/lint/bug workflows
      SKILL.md
      references/    # Workflow details loaded on demand
    qmd/             # Skill wrapper for semantic search
    obsidian-*/      # Obsidian helper skills
    defuddle/        # Web-page extraction helper
.claude/             # Legacy Claude Code prompts/settings kept for reference during migration
AGENTS.md            # This file — wiki schema + conventions (read by every command)
README.md            # User-facing docs
bugs.md              # Operator-logged issues from bug-report workflow sessions — read before editing workflows
...
Codex workflows live in `.agents/skills/llm-wiki/`:
- `SKILL.md` is the routing and shared operating guide.
- `references/*.md` contains the detailed workflows for setup, use, ingest, wiki-only lookup, web-augmented query, lint, and bug reporting.
- Steps are numbered and followed literally by the agent — keep ordering explicit.
- Every workflow starts by discovering the KB root (see below) before doing work. Don't skip that preamble when adding new workflows.
- Before changing a workflow, check `bugs.md` — past fixes are recorded there and the workflows encode those lessons.

## KB root resolution (all commands)

The KB a workflow operates on is determined by `.agents/active-kb` — a single-line file containing the basename of the active KB folder (e.g. `term-int-foly-inf`). The basename is also the QMD collection name; they are 1:1.

The pointer is local to each clone (gitignored — see `.gitignore`). It is written only by the active-KB switch workflow and by the setup workflow. No other workflow writes it. During migration, `.claude/active-kb` may be read as a legacy fallback if `.agents/active-kb` is missing, but Codex workflows do not write `.claude/active-kb`.

Every workflow that operates on a KB starts with this **resolution rule** — this section is the canonical version; the `llm-wiki` skill references it by name rather than duplicating it:

1. **Read `.agents/active-kb`** if it exists. If it does not exist, read `.claude/active-kb` as a legacy fallback if present. Trim the contents and treat them as `<kb-name>`. If `<kb-name>` is empty, contains a slash, or starts with `.` or `~`, print `.agents/active-kb should contain a single basename like 'term-int-foly-inf'; got '<value>'. Use the active-KB workflow to fix.` and stop. Otherwise, if `<project-root>/<kb-name>/wiki-config.json` exists, set `<root>` to that directory and read `<collection>` from the JSON. Done.
2. **Stale pointer.** If the pointer exists and is well-formed but `<project-root>/<kb-name>/` or its `wiki-config.json` is gone, print a one-line notice (`Active-KB pointer is stale: <kb-name> not found.`) and continue to step 3.
...
The ingest, query, wiki-only, lint, and setup workflows call the [QMD](https://github.com/tobilu/qmd) (`@tobilu/qmd`) CLI for local semantic search over `<kb-root>/wiki/`. Install once:
...
Use the `llm-wiki` skill explicitly (`$llm-wiki`) or ask in natural language:
...
- Use `[name]` — Switch `.agents/active-kb` to `<name>`, or with no argument print the current pointer and detected KBs.
